...
# if keys not generated already
if [ ! -f /vpn-certs/.certs_done ]; then
    ...
    if [ -n "${HOST}" ]; then
        ...
        # ++ certs generate - done
        echo  "done" > /vpn-certs/.certs_done
...
# ++ ipseconfig
cp /etc/ipsec_template.conf /etc/ipsec.conf
sed -i 's/@server_name_or_ip/'${HOST}'/g' /etc/ipsec.conf
sed -i 's/@subnet/'${SUBNET}'/g' /etc/ipsec.conf
...
iptables -A FORWARD --match policy --pol ipsec --dir in  --proto esp -s ${SUBNET}.0/24 -j ACCEPT
iptables -A FORWARD --match policy --pol ipsec --dir out --proto esp -d ${SUBNET}.0/24 -j ACCEPT
iptables -t nat -A POSTROUTING -s ${SUBNET}.0/24 -o eth0 -m policy --pol ipsec --dir out -j ACCEPT
iptables -t nat -A POSTROUTING -s ${SUBNET}.0/24 -o eth0 -j MASQUERADE
iptables -t mangle -A FORWARD --match policy --pol ipsec --dir in -s ${SUBNET}.0/24 -o eth0 -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss 1361:1536 -j TCPMSS --set-mss 1360
